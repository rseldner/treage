...
name: treage-agent
description: >
  Traverse an existing Treage TREE object as an agent to walk decision logic and
  produce a structured outcome. Use when given a Treage tree and a support case,
  ticket, log, or other input context to route through it.
metadata:
  author: rseldner
  version: 1.0.0
  treage-engine: "1.4.0"
...
For the full CONFIG and TREE schema, see [references/GRAMMAR.md](references/GRAMMAR.md).
To generate new Treage trees, see the **treage-build** skill.
...
This skill intentionally does not prescribe what happens after an outcome is reached. That is left to the operator. Some patterns:
...
To adjust traversal behavior for your deployment, modify `CONFIG.agentMode` in the tree file — no changes to this skill are needed.
